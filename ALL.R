...
print(paste("估算出的代理变量数量:", svobj$n.sv))
...
ggsave("SVA_PCA_Comparison.pdf", combined_plot, width = 12, height = 5.5, dpi = 300)
ggsave("SVA_PCA_Comparison.png", combined_plot, width = 12, height = 5.5, dpi = 300)
...
write.table(degIgG4, file = "degIgG4_change.txt",sep = "\t",row.names = T,col.names = NA,quote = F)
#ANCA
library(pheatmap)
...
ggsave("ANCA热图.pdf", p1, width = 12, height = 11.5, dpi = 300)
ggsave("ANCA热图.png", p1, width = 12, height = 11.5, dpi = 300)
ggsave("IGG4热图.pdf", p2, width = 12, height = 11.5, dpi = 300)
ggsave("IGG4热图.png", p2, width = 12, height = 11.5, dpi = 300)
...
this_tile <- paste0('Volcano plot of IgG4-RD',round(logFC,3),
                    '\nThe number of up gene is ',nrow(degANCA[degIgG4$change =='up',]) ,
                    '\nThe number of down gene is ',nrow(degANCA[degIgG4$change =='down',])
)
g2 = ggplot(data=degIgG4,
           aes(x=logFC, y=-log10(P.Value),
               color=change)) +
  geom_point(alpha=0.4, size=1.75) +
  theme_bw(base_size = 7) +
  ...
  scale_colour_manual(values = c('blue','black','red'))
print(g2)
ggsave("ANCA火山图.pdf", g1, width = 12, height = 10, dpi = 300)
ggsave("IGG4火山图.pdf", g2, width = 12, height = 10, dpi = 300)
...
geneCoef <- geneCoef[geneCoef$Gene != '(Intercept)',]
geneCoef
save(geneCoef,file = "geneCoef.Rdata")
write.table(geneCoef, file = "geneCoef.txt",sep = "\t",row.names = T,col.names = NA,quote = F)
...
ggplot(dataITGAM,aes(x =Group,y=dataITGAM,color=Group))+
  ...
  stat_compare_means(comparisons=my_comparison,size=13,#p.format只显示p值不显示检验方法，p.signif显示显著性水平符号
...
geneList <- res_ITGAM$t#t检验
names(geneList) <- rownames(res_ITGAM)
geneList <- geneList[!is.na(names(geneList))]# 移除NA并按降序排列
...
if (nrow(significant_pathways) > 10) {
  significant_pathways <- significant_pathways[1:10, ]  # 取前10条
}
#可视化
...
group_colors <- c("black", "black")  # 蓝色和红色，可按需调整
...
ggplot(cell_long, aes(Cell, value, fill = group)) +
  ...
  stat_compare_means(aes(group = group, label = ..p.signif..),
                     method = "kruskal.test",
                     symnum.args = list(
                       cutpoints = c(0, 0.001, 0.01, 0.05, 0.075, Inf),  # 注意 Inf 结尾
                       symbols = c("****", "***", "**", "*", "ns")
                     ))
